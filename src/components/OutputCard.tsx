...
export default function OutputCard({ content, label = 'Generated Content', onRegenerate, isLoading = false }: OutputCardProps) {
  ...
  const getCopyButtonColor = () => {
    switch (copyStatus) {
      case 'copied':
        return 'bg-emerald-700 border-emerald-600 hover:bg-emerald-700 hover:border-emerald-600';
      case 'failed':
        return 'bg-rose-700 border-rose-600 hover:bg-rose-700 hover:border-rose-600';
      default:
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          <button
            ...
            className={`inline-flex items-center justify-center rounded-full border px-4 py-2.5 text-xs font-medium text-slate-200 transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-card sm:px-5 sm:py-3 sm:text-sm ${getCopyButtonColor()}`}
          >
            {getCopyButtonText()}
          </button>
          <button
            ...
            disabled={isLoading}
            title={isLoading ? 'Generating...' : 'Generate new content'}
